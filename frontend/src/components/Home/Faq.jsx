...
const faqs = [
{
  ...
  const box = "border-2 border-deep-500 p-3 rounded-xl my-2 bg-blue-50";

  return (
    <div className="relative z-10 mt-10 flex flex-col gap-y-2 justify-center items-center">
      <center>
        <h1 className="text-4xl font-bold my-5 text-deep-500">FAQs</h1>
      </center>
      {faqs.map((faq, index) => (
        <div key={index} className={`${box} w-full`}>
          <div onClick={() => toggleDesc(index)}>
            <div className="flex justify-between items-center">
              <h2 className="font-bold ml-3 mb-3 mt-2 text-deep-500">{faq.question}</h2>
            ...
            <p
              className={`ml-3 mb-1 transition-[max-height,opacity] duration-300 ease-in-out overflow-hidden ${
